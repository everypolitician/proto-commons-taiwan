...
def town_reg_constituency_map(constituencies):
...
def create_attributes(constituencies_json):
    ...
    for c in constituencies_json:
        if c['constituency_type'] == 'regional':
            ...
            a[constituency] = {}
            a[constituency]['WIKIDATA'] = c['wikidata_item'].split('/')[-1]
            a[constituency]['MS_FB_PARE'] = county_msfb_map[c['county']]
            a[constituency]['MS_FB'] = '{}/county-constituency:{}'.format(
                attributes[constituency]['MS_FB_PARE'],
...
if __name__ == '__main__':
    ...
    towns_to_constituencies = town_reg_constituency_map(constituencies_json)
            ...
            for feature in districts_shp:
                try:
                    feature['geometry'] = fiona.transform.transform_geom(
                        'EPSG:3824', 'EPSG:4326', feature['geometry'])
                except Exception as e:
                    logging.exception("Error transforming feature {}"
                                      .format(feature['id']))
                ...
                try:
                    ...
                    logging.exception(
                        "Error calculating attributes for feature {}".format(
                            feature['id']
                        )
                    )
                ...
                try:
                    ...
                    logging.exception("Error writing feature {}"
                                      .format(feature['id']))
    ...
    with fiona.open(output_towns_shp_path, 'r') as towns_shp:
        ...
        with fiona.open(output_constituency_shp_path, 'w',
            ...
            feat_sorted = sorted(towns_shp,
                                 key=lambda k: k['properties']['CONSTIT_ID'])

            for key, group in itertools.groupby(feat_sorted,
                                                key=lambda x: x['properties']['CONSTIT_ID']):
                properties_list, geoms = zip(*[(feature['properties'],
                ...
                constituencies_shp.write({'geometry': geom,
